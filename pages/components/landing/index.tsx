...
export const Landing: React.FC = () => {
    ...
    useEffect(() => {
        const arr:HTMLCollectionOf<Element>  = document.getElementsByClassName('landing_section_container')
        Array.from(arr).forEach((element:Element) => {
            const key: Array<string> = Object.keys(obj).filter(value => element.className.indexOf(value) > -1)
            try{
                if(key.length > 0) {
                    // @ts-ignore
        ...
        window.onscroll = () => {
            ...
            let bool: boolean = false // if scroll top is in any range, set up as true.
            const scrollTop = document.documentElement.scrollTop + headerHeight
            Object.values(obj).forEach((value, index) => {
                // console.log('onscroll', scrollTop, keys[index], value)
                if(value?.start && value?.end && (scrollTop >= value?.start && scrollTop < value?.end)){
    ...
    const activatedTab = (hash: string) => uiState.hash === hash ? 'active' : ''
    ...
    const renderFooterLink = (value: any): ReactNode => {
        if(value.hasOwnProperty('title') && value.hasOwnProperty('link') ){
            ...
            counter = counter + 1
            return <div key={oldCounter} className={'sub_title'} onClick={() => window.open(value?.link, '_blank')}>{value?.title}</div>
        } else if (value instanceof Array && value.length === 2) {
            const oldCounter = counter
            counter = counter + 2
            return <div key={oldCounter} className={'category'}>
                <div key={oldCounter+1} className={'title'}>{value[0]}</div>
                {Object.entries(value[1]).map(value1 => renderFooterLink(value1[1]))}
            </div>
        } else {
    ...
    return <div className={"landing"}>
        ...
        >
            ...
            <div className={"menu"}>
                <div className={`tab ${activatedTab(Menu.Product)}`} onClick={() => onClickTab(Menu.Product)}>{Menu.Product}</div>
                {/*<div className={`tab`}>{Menu.Mission}</div>*/}
                <div className={`tab ${activatedTab(Menu.Technology)}`} onClick={() => onClickTab(Menu.Technology)}>{Menu.Technology}</div>
                <div className={`tab ${activatedTab(Menu.Team)}`} onClick={() => onClickTab(Menu.Team)}>{Menu.Team}</div>
                {/*<div className={`tab ${activatedTab(Menu.Community)}`} onClick={() => onClickTab(Menu.Community)}>{Menu.Community}</div>*/}
                <div className={"tab"} onClick={() => window.open('https://docs.cexiswap.io/token/cexi-scex-token', '_blank')}>Token</div>
                <div className={"tab"} onClick={() => window.open('https://docs.cexiswap.io/', '_blank')}>Docs</div>
            </div>
            <div className={"navigation"}>
                {/*<div className={"tab coming_soon"}>Agent Center</div>*/}
                <div className={`tab heart_beat`} onClick={() => window.open('https://www.cexiswap.io/airdrop', '_blank')}>Air Drop</div>
                <div className={`tab`} onClick={() => window.open('https://app.cexiswap.io/', '_blank')}>Trade</div>
            </div>
        </div>
        ...
        <div className={"landing_section_container landing_section_4"}>
            ...
            <div className={'container'}>
                ...
                <div className={'title2'}>Exploitation & Worse DEX Prices Caused by Trapped Liquidity</div>
                <div className={'gap'} />
                <div className={'des'}>Multiple new layer one and layer two blockchain solutions create fragmentation of DeFi application ecosystems and the trapped, captive liquidity in their respective domains.</div>
            </div>
            <div className={'space'}/>
        </div>
        <div className={"landing_section_container landing_section_9"}>
            ...
            <div className={'container'}>
                ...
                <div className={'sub_title'}>We Disrupt Blockchain Boundaries Liberate CEX & DEX Liquidity to Every Chain</div>
                <div className={'gap'} />
                <div className={'des'}>CEXISWAP’s mission is to eliminate the exploitation and fulfill the DeFi 2.0 vision. </div>
            </div>
            <div className={'space'}/>
        </div>
        <div className={`landing_section_container landing_section_2 ${Menu.Product}`}>
            <div className={'header'}>
                <div className={"title"}>{Menu.Product} Features</div>
                <div className={"description"}>CEXISWAP is a decentralized exchange that liberates DEX and CEX liquidity.</div>
            </div>
                ...
                <div className={'flex'}>
                    <div className={'detail'}>
                        <div className={'detail_icon'}>
                            ...
                            <div className={'des'}>Competitive prices through a decentralized network</div>
                        </div>
                        <div className={'detail_icon'}>
                            <div className={'icon icon2'}/>
                            <div className={'t'}>Better experience than CEX</div>
                            <div className={'des'}>No deposits are required. Trade directly from your wallet</div>

                        </div>
                        <div className={'detail_icon'}>
                            <div className={'icon icon3'}/>
                            <div className={'t'}>Zero slippage</div>
                            <div className={'des'}>See the final price before trading with instant and reliable price quotes</div>
                        </div>
                        <div className={'detail_icon'}>
                            <div className={'icon icon4'}/>
                            <div className={'t'}>Front-run protection</div>
                            <div className={'des'}>Protection against MEV (sandwiching, arbitrage, and front running)</div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div className={"landing_section_container landing_section_10"}>
            ...
            <div className={'container'}>
                <div className={'problem_section'}>
                    ...
                    <div className={'sub_title'}>CEXISWAP is developed as an efficient, ready-to-use solution, bridging the gap between CEXs and DEXs, promising technology and mass crypto users.</div>
                    <div className={'bg4'}/>
                </div>
            </div>
            <div className={'space'}/>
        </div>
        <div className={`landing_section_container landing_section_3 ${Menu.Technology}`}>
            <div className={'title'}>{Menu.Technology}</div>
            <div className={'section'}>
                <div className={'left_section'}>
                    <div className={'description'}>
                        <div className={'t'}>Trade Pipe Technology (TPT)</div>
                        <div className={'sub_t'}>In order to fulfill our mission of liberating</div>
                        <div className={'sub_t'}>liquidity, we built our Trade Pipe Technology</div>
                        <div className={'sub_t'}>(TPT) into a DEX to execute user orders</div>
                        <div className={'sub_t'}>based on the trading environment and</div>
                        <div className={'sub_t'}>demand.</div>
                        <div className={'git_book'} onClick={() => window.open('https://docs.cexiswap.io/cexiswap/trade-pipe-technology-tpt', '_blank')}>GitBook</div>
                    </div>
                </div>
        ...
        {/*    </div>*/}
        {/*</div>*/}
        <div className={`landing_section_container landing_section_5 ${Menu.Team}`}>
            ...
            <div className={'profile_section'}>
                <div className={'profile_container'}>
                    ...
                    <div className={'linked_icon'} onClick={() => window.open('https://www.linkedin.com/in/scotttsai/', '_blank')}/>
                </div>
                <div className={'profile_container'}>
                    <div className={'photo Cheryl_Chueh'} />
                    <div className={'name'}>Cheryl Chueh</div>
                    <div className={'job_title'}>Co-Founder & CMO</div>
                    <div className={'linked_icon'} onClick={() => window.open('https://www.linkedin.com/in/cheryl-chueh/', '_blank')}/>
                </div>
                <div className={'profile_container'}>
            ...
            <div className={'profile_section'}>
                <div className={'profile_container'}>
                    ...
                    <div className={'job_title'}>
                        ...
                        <div className={'linked_icon'} onClick={() => window.open('https://www.linkedin.com/in/chris0804/', '_blank')}/>
                    </div>
                </div>
                ...
                <div className={'profile_container'}>
                    ...
                    <div className={'job_title'}>
                        ...
                        <div className={'linked_icon'} onClick={() => window.open('https://www.linkedin.com/in/kungao/', '_blank')}/>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div className={"landing_section_container landing_section_7"}>
            ...
            <div className={'section'}>
                ...
                <div className={'sub_title'}>Create a new world without exploitation</div>
                <div className={'question'}>Do you accept?</div>
                <div className={'flex'}>
                    <div className={'join'}  onClick={() => window.open('https://app.cexiswap.io', '_blank')}>Trade Now</div>
                </div>
            </div>
            <div className={'flex1'} />
            <div className={'footer'}>
                <div className={'social_section'}>
                    <div className={'social social1'} onClick={() => window.open('https://t.me/cexiswap_official', '_blank')}/>
                    <div className={'social social2'} onClick={() => window.open('https://twitter.com/cexiswap', '_blank')}/>
                    <div className={'social social3'} onClick={() => window.open('https://www.facebook.com/cexiswap', '_blank')}/>
                </div>
            </div>
        </div>
